...
```bash
stand-alone-lambda$ sam local invoke helloFromLambdaFunction --no-event
```
...
Resources:
  ...
  helloFromLambdaFunction:
    ...
    Properties:
      CodeUri: ./src/stand-alone-lambda/
      Handler: stand-alone-lambda::stand-alone-lambda.Function::FunctionHandler
...
```bash
stand-alone-lambda$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
stand-alone-lambda$ dotnet test test/stand-alone-lambda.Tests/stand-alone-lambda.Tests.csproj 
```
...
```bash
aws cloudformation delete-stack --stack-name stand-alone-lambda
```
